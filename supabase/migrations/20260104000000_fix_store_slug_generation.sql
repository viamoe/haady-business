-- ================================
-- Fix Store Slug Generation
-- ================================
-- The previous slug generation had a bug where LOWER() was applied AFTER
-- the regex replace, causing uppercase letters to be removed instead of
-- converted to lowercase.
--
-- Example bug: "Nike Store" → "ike-tore" (N and S were removed)
-- Correct:     "Nike Store" → "nike-store-4332" (with first 4 chars of UUID for uniqueness)

-- ================================
-- Step 1: Fix existing slugs by regenerating them from store names
-- ================================
DO $$
DECLARE
  store_record RECORD;
  ...
  uuid_prefix TEXT;
BEGIN
  -- Loop through all stores that have a name but potentially broken slug
  FOR store_record IN 
  ...
  LOOP
    ...
    -- Update the store with correct slug (skip if same as current)
...
-- ================================
-- Step 2: Update the RPC function with correct slug generation
-- ================================
...
BEGIN
  ...
  -- Generate slug from store name (FIXED: LOWER first, then regex)
  ...
  -- Note: UUID prefix will be added after store creation (see below)
...
-- Add comment
COMMENT ON FUNCTION public.save_store_details_onboarding_step IS 'Saves store details during onboarding. Fixed slug generation to properly lowercase before regex.';
