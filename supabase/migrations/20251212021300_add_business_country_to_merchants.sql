-- ================================
-- Add business_country to merchants table
-- ================================
-- Description: Update create_merchant_onboarding RPC to set merchants.country
--              from the selected country in the setup form
-- Date: 2024-12-12
...
-- Create the RPC function with business_country support
...
BEGIN
  ...
  v_business_country := v_preferred_country; -- Use preferred_country as business_country
  ...
  IF v_existing_merchant_user_id IS NOT NULL THEN
    ...
    -- Create merchant with business_country (country field)
...
COMMENT ON FUNCTION public.create_merchant_onboarding IS 'Creates a merchant account and links it to the authenticated user. Sets merchants.country from the selected country in the setup form. Phone number is stored in merchant_users.phone, not merchants.contact_phone.';
