-- ================================
-- Add summary step to onboarding flow
-- ================================
-- Description: Updates onboarding_step enum to include 'summary' and updates RPC functions
-- Date: 2025-12-21

-- =====================================================
-- PART 1: Update onboarding_step check constraint to include 'summary'
-- =====================================================
...
BEGIN
  ...
  DROP CONSTRAINT IF EXISTS business_profile_onboarding_step_check;
  
  -- Add new constraint with 'summary' included
  ALTER TABLE public.business_profile
  ADD CONSTRAINT business_profile_onboarding_step_check
  CHECK (onboarding_step IS NULL OR onboarding_step IN ('personal-details', 'business-setup', 'connect-store', 'summary', 'completed'));
  
  RAISE NOTICE 'Updated onboarding_step constraint to include summary';
END $$;

-- =====================================================
-- PART 2: Update save_store_connection_onboarding to set step to 'summary'
-- =====================================================
...
BEGIN
  ...
  SET 
    onboarding_step = 'summary', -- Move to summary step
    updated_at = NOW()
  WHERE id = v_business_profile_id;
...
COMMENT ON FUNCTION public.save_store_connection_onboarding IS 'Step 3 of onboarding (optional): Saves external platform connection to store_connections table and sets onboarding_step to summary';
GRANT EXECUTE ON FUNCTION public.save_store_connection_onboarding TO authenticated;

-- =====================================================
-- PART 3: Add RPC to set onboarding_step to summary when skipping
-- =====================================================

CREATE OR REPLACE FUNCTION public.skip_store_connection_onboarding()
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Update onboarding_step to 'summary' (move to summary step)
  UPDATE public.business_profile
  SET 
    onboarding_step = 'summary', -- Move to summary step
    updated_at = NOW()
...
COMMENT ON FUNCTION public.skip_store_connection_onboarding IS 'Moves user to summary step when they skip store connection';
GRANT EXECUTE ON FUNCTION public.skip_store_connection_onboarding TO authenticated;
...
BEGIN
  RAISE NOTICE '==============================================';
  RAISE NOTICE 'Added summary step to onboarding';
  RAISE NOTICE '  - Updated onboarding_step constraint to include summary';
  RAISE NOTICE '  - Updated save_store_connection_onboarding to set step to summary';
  RAISE NOTICE '  - Added skip_store_connection_onboarding RPC';
  RAISE NOTICE '==============================================';
END $$;
