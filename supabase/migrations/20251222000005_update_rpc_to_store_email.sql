-- ================================
-- Update RPC to store email
-- ================================
-- Description: Updates save_store_connection_onboarding to accept and store email
-- Date: 2025-12-22
...
CREATE OR REPLACE FUNCTION public.save_store_connection_onboarding(
  ...
  p_email TEXT DEFAULT NULL  -- Store email from external platform
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.save_store_connection_onboarding IS 'Step 3 of onboarding: Saves external platform connection. Accepts optional p_user_id for server-side calls, p_external_store_name for the platform store name, and p_email for the store email.';
