-- ================================
-- Update create_merchant_onboarding RPC to use country_id
-- ================================
-- Description: Update the RPC function to store country_id (UUID) instead of country (TEXT)
--              The function will look up the country_id from countries_master based on ISO2 code
-- Date: 2024-12-12
...
-- Create the RPC function with country_id support
...
BEGIN
  ...
  -- If not found in countries_master, try countries table
  ...
  IF v_country_id IS NULL THEN
    ...
    IF v_country_id IS NULL THEN
      ...
      -- If still not found, try countries table
      IF v_country_id IS NULL THEN
        SELECT id INTO v_country_id
        FROM public.countries
        ORDER BY name
...
COMMENT ON FUNCTION public.create_merchant_onboarding IS 'Creates a merchant account and links it to the authenticated user. Sets merchants.country as UUID foreign key referencing countries_master.id. Phone number is stored in merchant_users.phone, not merchants.contact_phone.';
