-- ================================
-- Add opening_hours column to stores and fix RPC logic
-- ================================
-- Description: Adds opening_hours column to stores table and fixes RPC to only set it when retail is in store_types
-- Date: 2025-12-21

-- =====================================================
-- PART 1: Add opening_hours column if it doesn't exist
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    COMMENT ON COLUMN public.stores.opening_hours IS 'JSON object containing store opening hours. Format: {day: {open: ''HH:MM'', close: ''HH:MM'', closed: boolean}}. Only set for retail/hybrid/pop_up store types.';
...
-- =====================================================
-- PART 2: Update RPC function to fix opening_hours logic
-- =====================================================
...
BEGIN
  ...
  IF v_store_id IS NOT NULL THEN
    ...
    SET 
      ...
      opening_hours = v_final_opening_hours, -- Only set if retail is in types
  ...
  ELSE
    ...
    VALUES (
      ...
      v_final_opening_hours, -- Only set if retail is in types
      p_logo_url,
      'haady',
      true
    )
...
BEGIN
  RAISE NOTICE '==============================================';
  RAISE NOTICE 'Fixed opening_hours column and RPC logic';
  RAISE NOTICE '  - Added opening_hours column to stores';
  RAISE NOTICE '  - Updated RPC to only set opening_hours when retail is in store_types';
  RAISE NOTICE '  - Store categories: multi-select (handled)';
  RAISE NOTICE '  - Store types: multi-select (handled)';
  RAISE NOTICE '';
  RAISE NOTICE 'Opening hours logic:';
  RAISE NOTICE '  - Only set when ''retail'' is in p_store_types array';
  RAISE NOTICE '  - NULL for online-only stores';
  RAISE NOTICE '==============================================';
END $$;
