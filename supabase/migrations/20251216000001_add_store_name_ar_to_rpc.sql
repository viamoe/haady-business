-- ================================
-- Add store_name_ar parameter to create_business_onboarding RPC
-- ================================
-- Description: Updates the RPC function to accept and save Arabic store name separately
-- Date: 2025-12-16

-- Drop the existing function
DROP FUNCTION IF EXISTS public.create_business_onboarding CASCADE;

-- Recreate the function with store_name_ar parameter
CREATE OR REPLACE FUNCTION public.create_business_onboarding(
  ...
  store_name_ar TEXT DEFAULT NULL,
  ...
  store_country UUID DEFAULT NULL,
...
BEGIN
  ...
  IF v_existing_profile_id IS NOT NULL THEN
    ...
    SET 
      ...
      business_country = v_business_country,
  ...
  -- Create store if store_name is provided
  IF store_name IS NOT NULL OR store_name_ar IS NOT NULL THEN
    -- Generate slug from store name (prefer English, fallback to Arabic)
    v_store_slug := LOWER(REGEXP_REPLACE(
      COALESCE(store_name, store_name_ar, 'store'),
      '[^a-z0-9]+', '-', 'g'
    ));
    ...
    -- Create store with both English and Arabic names
    ...
    VALUES (
      ...
      store_name, -- English name (can be NULL)
      store_name_ar, -- Arabic name (can be NULL)
      v_store_slug,
      store_city,
      store_country,
      store_address,
  ...
  RETURN json_build_object(
    ...
    'store_id', COALESCE(v_store_id, NULL)
  );
END;
$$;

COMMENT ON FUNCTION public.create_business_onboarding IS 'Creates or updates business profile with business information, including separate English and Arabic store names';
