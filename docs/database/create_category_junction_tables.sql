-- ====================================================================
-- CREATE CATEGORY JUNCTION TABLES
-- Description: Links categories to stores and products
-- NOTE: Run this AFTER categories, business_profiles, and products tables exist
-- Date: December 2024
-- ====================================================================
...
CREATE POLICY "Store owners can manage their store categories"
  ...
  USING (
    EXISTS (
      ...
      WHERE business_profile.id = store_categories.store_id
      AND business_profile.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Store owners can manage their product categories"
  ...
  USING (
    EXISTS (
      ...
      JOIN business_profile ON products.business_id = business_profile.id
